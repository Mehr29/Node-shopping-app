  ...
  User.findOne({email:email}).then(user=>{
    ...
    bcrypt.hash(password,12).then(hashedpassword=>{
      ...
      return transporter.sendMail({
                to: email,
                from: 'suchismita.dash.bina@gmail.com',
                subject: 'Successful Signup',
                html: '<h1> Succesful Signup Confirmed</h1>'
              })
    }).catch(err=>console.log(err))
  }).catch(err=>{
    console.log(err)
  ...
  crypto.randomBytes(32,(err,buffer)=>{
    ...
    User.findOne({email:email}).then(user=>{
      ...
      return transporter.sendMail({
        ...
        html: `<h1> Reset your password <a href="http://localhost:3000/reset/${token}">here</a></h1>`
      })
    })
    .catch(err=>console.log(err));
  })
...
exports.getNewPassword = (req, res, next) => {
  ...
  User.findOne({ refreshToken: token, refreshTokenExpiration: { $gt: Date.now() } })
    .then(user => {
      let message = req.flash('error');
      if (message.length > 0) {
        message = message[0];
